...
function GradesList({items,source,limit,onMore}:{items:CollectionCatalogItem[];source:RatingSource;limit:number;onMore:()=>void}){
  ...
  return <><div className="grades-sortbar"><span>Sort by</span><div>{headings.map(([key,label])=><button type="button" className={sortState.key===key?"is-active":""} onClick={()=>changeSort(key)} aria-label={`Sort ${label} ${sortState.key===key&&sortState.direction==="desc"?"ascending":"descending"}`} key={key}><span>{label}</span>{sortState.key===key?(sortState.direction==="desc"?<ChevronDown aria-hidden="true"/>:<ChevronUp aria-hidden="true"/>):null}</button>)}</div></div><div className="grades-card-grid">{shown.map((item)=>{const rating=item.ratings[source];return <article className="grade-card" key={item.id}><div className="grade-character"><Portrait compact item={item} source={source}/></div><dl className="grade-card-metrics"><div><dt>Rating</dt><dd><RatingBadge rating={rating?.rating}/></dd></div><div><dt>Grinding</dt><dd><GradeBadge value={rating?.grinding}/></dd></div><div><dt>Full Auto</dt><dd><GradeBadge value={rating?.fullAuto}/></dd></div><div><dt>High difficulty</dt><dd><GradeBadge value={rating?.highDifficulty}/></dd></div></dl></article>})}</div>{shown.length<items.length&&<div className="collection-load-more"><Button variant="outline" onClick={onMore}>Show 120 more</Button><span>{shown.length} of {items.length}</span></div>}</>;
}
