# GBF Dark Magna Roster Plan

Last updated: 2026-08-18

## Account direction

- Remain **Dark Magna**. This account lacks the premium characters and weapon inventory that would justify Hades, and it does not own Ereshkigal.
- Missing Summer Catura and Grand Sariel removes top-end normal-attack speed, but it does not prevent reliable casual Full Auto.
- The account owns a coherent modern sustain/skill core: **Grand Lich, Summer Magisa and Tsukuyomi**.
- It also owns **Yukata Seofon, Summer Vajra and Grand Shalem**, which form a separate CA/control team rather than a weaker copy of the Lich team.
- The account currently has no listed Seox. Recruiting and eventually transcending Seox is the largest deterministic character project.

## Roster mapping

The supplied payload contains 35 Dark SSRs.

### Important owned characters

- **Grand Lich:** primary owned general Full Auto attacker/debuffer. Repeated automatic skill damage, delays, debuffs and strong compatibility with Magna skill-damage weapons keep her central to the current plan.
- **Tsukuyomi:** premier owned healer/defensive support. Her permanent Moonlight Blessing supplies damage reduction and debuff-duration reduction; crest-granting skills trigger team healing and charge-bar support.
- **Summer Magisa:** crest/CA/skill attacker and dispeller. She generates Oblivion Crests, has repeated multi-hit damage and dispel access, and directly activates Tsukuyomi's healing/gauge engine.
- **Yukata Seofon:** medium/long Full Auto attacker/support. He consumes charge bar before normal attacks to build Bladeshine, has guaranteed TA through that process, and uses his separate Windmill resource for unworldly CAs and defensive utility. He needs ramp time.
- **Summer Vajra:** CA-oriented attacker/support suitable for the second team.
- **Grand Shalem:** permanent debuffs, CA/control value and high-difficulty utility. She is older but still useful when the team needs actual boss control rather than only damage.
- **Tyra:** useful modern skill/normal attacker. She becomes particularly relevant if Grand Orologia is acquired; Japanese GameWith used Tyra/Lich/Orologia in a Magna 150-HELL Manadiver example.
- **Constance:** regular-pool long Full Auto attacker/support. When protected from damage she automatically deals multi-hit skill damage, debuffs and heals, while building permanent team offense/defense/MA/skill support. She also supplies veil, dispel cancel and Light-damage reduction. Useful, but below the Lich/Magisa/Tsukuyomi core.

### Offensive and situational depth

- **Summer Azusa:** high personal multiplier/cap with an accuracy ramp, automatic healing when successful hits consume stacks, and good skill/CA contribution. Useful in short farming and as an offensive replacement.
- **Summer Hallessena:** dodge-based attacker and CA support; useful in specific long/evasive fights.
- **Summer Meg:** normal attacker whose value improves with other summer characters. The roster can satisfy her seasonal condition, but the result is less broadly reliable than the Lich team.
- **Cendrillon:** short/specialized offensive option, not a general Full Auto foundation.
- **Narmaya 5-star:** usable older normal/CA attacker, but no longer the first modern damage choice.
- **Grand Rei:** uniquely powerful when manual zero-turn switching or double-strike targeting is allowed. Strict one-button Full Auto does not automatically execute her zero-turn setup skills, so she is not a default unattended preset.
- **Grand Ferry:** older support with healing, gauge, debuffs and dispel; a valid replacement when more sustain/control is required.
- **Yukata Seofon, Tsukuyomi, Dark Grimnir, Dark Florence, Summer Magisa, Lich, Grand Shalem, Grand Ferry, Rei, Summer Azusa, Summer Hallessena, Summer Meg, Summer Vajra, Constance and Tyra** are the roster's meaningful modern/specialized depth.
- Dark Albert, Zehek, Holiday Fiorito, Lucius, Black Knight, Dark Zeta, Summer Tabina, Predator, Marquiares, Lunalu, Kolulu, Tanya, Ippatsu, Meg & Mari, Dark Grimnir, Nicholas, Grimnir and older characters should receive resources only for a demonstrated encounter need.

## Team 1: default casual skill/sustain Full Auto

**Grand Lich / Summer Magisa / Tsukuyomi**

- CA: **On**.
- This is the account's strongest current general-purpose Dark team.
- Magisa produces Oblivion Crests; Tsukuyomi converts crest-granting skill casts into healing and charge bar while providing permanent damage reduction and debuff-duration reduction.
- Lich supplies automatic skill damage, delays and debuffs. The team's numerous skill hits benefit strongly from Ater Katana, Agonize and skill-cap/supplemental effects.
- Manadiver adds repeated skill damage and can provide additional crests depending on main weapon/setup.

Substitutions:

- Replace Magisa with **Constance** when veil, dispel cancel, Light-damage reduction and longer-ramping team buffs matter more than crest speed.
- Replace Magisa or Lich with **Tyra** when the boss is easy and additional personal damage is preferable.
- Replace Tsukuyomi with **Grand Ferry** if her debuff/dispel package specifically solves the fight, but Tsukuyomi is normally the better unattended sustain member.
- Replace a damage slot with **Dark Grimnir** for an all-ally substitute/emergency tank turn.

## Team 2: Seox general Full Auto

**Seox / Grand Lich / Tsukuyomi**

- CA: **On**.
- Seox is the primary self-sufficient normal/hybrid attacker. Lich supplies automatic damage, delays and debuffs, while Tsukuyomi makes the composition safe enough for unattended use.
- Replace Tsukuyomi with Summer Magisa when survival is solved. Grand Sariel is the future offensive upgrade.
- Use three Ater Fists and one Ater Katana so the grid retains HP, CA and specialty support instead of forcing five boost weapons.

Summer Azusa remains Team 1's offensive skill flex:

**Summer Azusa / Grand Lich / Summer Magisa**

For one-turn skill damage, enable Azusa's first and second skills and disable unnecessary party abilities. This uses Team 1's three-Fist/two-Katana skill grid and does not require Kaneshige.

## If Grand Orologia is acquired

Orologia is not merely a rating chase for this roster. The account already owns the pieces that make the acquisition immediately useful.

Recommended general Full Auto:

**Tyra / Grand Lich / Grand Orologia**

- CA: **On**.
- This closely follows a current Japanese Magna Full Auto example used for 150-HELL.
- Orologia supplies early guaranteed TA/personal strength, permanent-ramping offense, party veil and five-turn Dark attack/critical/damage-cap/damage-dealt support, plus emergency recovery behavior.
- Tyra and Lich capitalize on the skill/normal support and give the team substantial automatic damage.

Safer Orologia version:

**Grand Lich / Grand Orologia / Tsukuyomi**

Use this for unfamiliar bosses or casual unattended farming. It sacrifices Tyra's speed for Tsukuyomi's damage reduction and healing.

Orologia would replace Summer Magisa or Tsukuyomi according to the encounter; Lich remains the most natural owned partner. Orologia does not supply Grand Sariel's Primarch passive, so Sariel remains a future target.

## What the three missing 10-rated characters actually do

### Grand Sariel

- Highest broad account value.
- Adds 20% to Dark/Oblivion/Malice/Misty Gloom Omega weapon skills from the frontline or backline, benefiting Magna and Hades.
- Strong opening attacker with repeated attacks, skill follow-ups and unique party echo.
- Future normal/general shell: **Grand Sariel / Seox / flex**, with Orologia, Summer Catura, Lich or a defensive unit filling the final role.

### Grand Sariel team ladder: premium to current reality

Sariel's best teammates depend on whether the objective is manual/very short burst or casual unattended Full Auto. His opening guaranteed TA, three attack actions and post-normal skill damage are disproportionately valuable on turn 1; his Primarch passive remains valuable forever, including from reserve.

#### Premium normal/general Full Auto (no Eresh required)

**Summer Catura / Grand Sariel / Grand Orologia**

- CA **Off** for short normal farming; **On** for medium/long Full Auto when ougi effects and stability matter.
- Catura concentrates support on herself and MC, while Sariel supplies unique echo, damage reduction and a powerful opening. Orologia adds veil, team damage/cap support and recovery.
- This is the premium casual normal-team destination. It does not require Ereshkigal, though it does require three limited characters and a developed grid.

Premium skill/general alternative:

**Grand Sariel / Grand Lich / Grand Orologia**

- CA **On**.
- Prefer this over the Fighter Origin team for bosses that reward automatic skill damage, delays, debuffs and dispels.

#### Realistic team immediately after acquiring Sariel

**Grand Sariel / Grand Lich / Tsukuyomi**

- CA **On**.
- This is the recommended default for the player's actual roster. Sariel and Lich provide damage, debuffs and skill follow-ups; Tsukuyomi supplies damage reduction, debuff-duration reduction, shields and healing.
- It is less explosive than the Catura/Orologia team but much more reliable than copying an Eresh composition without its engine.

Faster owned variation:

**Grand Sariel / Grand Lich / Summer Magisa**

- CA **On**.
- Use when the boss is easy enough to give up Tsukuyomi's sustain. Magisa adds crest scaling, multi-hit damage and dispels.

Short owned variation:

**Grand Sariel / Summer Azusa / Cendrillon**

- Usually CA **Off** or configured specifically for the target.
- This approximates a low-input opening-damage composition. Do not invest heavily merely to optimize this temporary version.

#### After deterministic Seox recruitment

**Grand Sariel / Seox / Grand Lich**

- Default offensive general team after Seox is developed.
- Replace Lich with Tsukuyomi for survival, or move Sariel to reserve if a particular mechanic requires three other frontline specialists. His 20% weapon-skill passive still applies.

Reality check: Sariel would be an excellent acquisition even if he spent many battles in the backline. The account does not need Summer Catura, Orologia or Ereshkigal before Sariel becomes worthwhile.

### Summer Catura

- Modern normal-team partner focused heavily on herself and MC.
- Particularly strong with Fighter Origin: she improves MC and herself, triggers dispel skill damage after their TAs, and rewards multiple attack actions.
- She is a normal/high-difficulty archetype upgrade, not a replacement for Lich's skill/debuff role.

## Current backline

This roster does not have a mandatory always-on reserve pair comparable to a Primarch plus completed Evoker.

- **Grand Rei** can provide CA support from reserve only after her permanent ougi debuff has been applied; her best switch setup requires manual zero-turn input, so do not count it as automatic from battle entry.
- **Summer Magisa**, when not frontline, can be useful in crest-centered configurations, but is normally more valuable in front with Tsukuyomi.
- Use the strongest emergency reinforcements: **Constance**, **Grand Ferry**, **Summer Azusa** or **Dark Grimnir**, chosen for sustain, damage or tanking.

Future default reserve:

1. **Grand Sariel**, whenever not frontline, for his 20% Primarch weapon-skill passive.
2. **Nier**, after Arcarum recruitment/development, or an encounter-specific emergency reinforcement.

## Arcarum and Eternal roadmap

1. **The Death summon:** immediate Dark progression and future swap utility. Its call enables controlled sacrifice strategies, but casual presets do not need to force them.
2. **Nier:** Dark's Evoker and the main Arcarum character project. Her developed frontline/entry value is more important than treating her solely as a passive backliner.
3. **Seox recruitment, 5-star and transcendence:** the highest deterministic character priority. Seox remains an elite self-sufficient Dark attacker and is a natural future partner for Grand Sariel.

## Magna grid direction

Default summon direction: **Celeste Omega 250 × Celeste Omega 250** for stable Full Auto.

General modern shell:

- **2–3 Ater Fists** at 4-star for Magna boost, attack and cap;
- **1–2 Ater Katanas** for skill damage, especially Team 1;
- **1–2 Ater Daggers** when healing/critical/Enmity-defense is needed for difficult Full Auto;
- **Magna Dark Opus**, eventually 5-star/6-star;
- **Agonize** for skill supplemental damage while using Lich/Magisa/Tsukuyomi/Constance;
- **Fediel Spines** or other CA-cap pieces for Team 2;
- **Agastia staff** attack awakening as a later farmed offensive flex;
- **Agastia harp** defense awakening for demanding 200/250-style Full Auto;
- **Ultima** and **Celestial** weapons when the active team's specialties justify them.

Do not Damascus-bar farmable Magna pieces. Do not copy Ereshkigal/Hades grids when the account is building unattended Magna Full Auto. Exact counts must wait for the Dark weapon payload.

## Acquisition priority

### Deterministic projects

1. Recruit and develop **Seox**.
2. Farm **The Death** and recruit/develop **Nier**.
3. Complete Celeste Omega 250 and the Magna III/Opus foundation.

### Gacha targets

1. **Grand Sariel:** highest overall Dark account value because his Primarch passive improves essentially every Dark grid even from reserve.
2. **Grand Orologia:** highest immediate frontline improvement for the owned Lich/Tyra roster. If a full 300-draw spark is already available on the current worthwhile Flash Gala, Orologia is a defensible spark rather than a desperation pick.
3. **Summer Catura:** premium future Fighter Origin/normal-team target. Her limited opportunity can move her ahead of Orologia when she is actually sparkable on a strong seasonal 6% Gala.
Banner pools matter: Sariel is a Legfest target, while Orologia is a Flashfest target, so they can be planned in parallel rather than treated as mutually exclusive forever.

## Practical conclusion

- Dark is not actually nonfunctional: the owned Lich/Magisa/Tsukuyomi combination is a strong casual Full Auto foundation.
- **Current Team 1:** Manadiver / Grand Lich / Summer Magisa / Tsukuyomi.
- **Future Team 2:** Manadiver / Seox / Grand Lich / Tsukuyomi, with Summer Magisa replacing Tsukuyomi when survival is solved.
- **Summer Azusa:** retained as Team 1's one-turn/offensive skill flex rather than requiring a separate Kengo preset.
- **After Orologia:** Manadiver / Tyra / Grand Lich / Grand Orologia, or Lich/Orologia/Tsukuyomi for safety.
- **Broad first target:** Grand Sariel.
- **Current-banner immediate upgrade:** Grand Orologia, if and only if a complete spark is available and the banner itself is worth finishing.
- **Seasonal target:** Summer Catura for the future normal/high-difficulty team.
- **Farm priority:** Seox, The Death and Nier.
