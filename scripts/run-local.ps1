...
$nodeExe = if ($env:SOUTENROKU_NODE -and (Test-Path -LiteralPath $env:SOUTENROKU_NODE)) {
  $env:SOUTENROKU_NODE
...
if ($major -lt 22) {
  throw "Soutenroku requires Node.js 22 or newer. Found $(& $nodeExe --version). Install Node 22+, or set SOUTENROKU_NODE to its node.exe path."
}
...
try {
  ...
  Write-Host 'Soutenroku will open at http://localhost:3000/' -ForegroundColor Cyan
